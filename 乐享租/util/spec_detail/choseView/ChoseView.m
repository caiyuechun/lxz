...
-(void)initTypeView:(NSArray *)dataSource specPriceArr:(NSDictionary *)priceArrs
{
    NSLog(@"数据源%@，%lu",dataSource,(unsigned long)dataSource.count);
    CGFloat hei = 0;
    specArrs = dataSource;
    priceArr = priceArrs;
    for(int i =0;i<dataSource.count;i++)
    {
        TypeView *specView = [[TypeView alloc] initWithFrame:CGRectMake(0, hei, self.frame.size.width, 50+50*i) andDatasource:dataSource[i][@"list"] :dataSource[i][@"name"]];
        specView.tag = i;
        specView.delegate = self;
        specView.frame = CGRectMake(0, specView.frame.origin.y, self.frame.size.width, specView.height);
        hei =specView.height+specView.frame.origin.y;
         [mainscrollview addSubview:specView];
        if(i==dataSource.count-1)
        {
        countView.frame = CGRectMake(0, specView.frame.size.height+specView.frame.origin.y, self.frame.size.width, 50);
...
{
    NSLog(@".....tag.....%d",tag);
    NSString *select = specArrs[0][@"list"][tag][@"item"];
    NSString *price = [priceArr objectForKey:select][@"price"];
    lb_price.text = price;
