...
- (void)sureBtnAction:(UIButton *)sender {
    ...
    if (assetArray.count > 5) {
        [WToast showWithText:@"图片上传的最多限制为5张"];
      //  [SVProgressHUD showInfoWithStatus:@"图片上传的最多限制为5张"];
        return;
    }
...
- (void)changeCell:(SJPickPhotoCell *)changedCell WithModel:(SJPhotoModel *)changedModel {
    ...
    if (self.pickedArray.count > 3) {
        if (changedModel.isPicked) {
            changedModel.isPicked = NO;
            [WToast showWithText:@"图片上传的最多限制为4张"];
            //  [self showHint:@"图片上传的最多限制为5张"];
            return;
        }
