...
-(void)viewWillAppear:(BOOL)animated{
    ...
    self.notiNum_Lab.layer.cornerRadius = 10;
...
-(void)loadData{
    ...
    [self postWithURLString:@"/index/home" parameters:parm success:^(id response) {
        ...
        if(response){
            ...
            [self.icon_Img sd_setImageWithURL:[NSURL URLWithString:self.datas[@"user"][@"head_pic"]] placeholderImage:nil];
            self.nick_Lab.text = self.datas[@"user"][@"nickname"];
//            //消息数量
...
//
//}
-(void)reloadViews:(NSNotification *)noti{
    NSDictionary *userDIc = noti.object;
    [self.icon_Img sd_setImageWithURL:[NSURL URLWithString:userDIc[@"head_pic"]] placeholderImage:nil];
    self.nick_Lab.text = userDIc[@"nickname"];
}
- (void)viewDidLoad {
    [super viewDidLoad];
    [self setStatusBarBackgroundColor:[UIColor whiteColor]];
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(reloadViews:) name:@"reloadNum" object:nil];
...
-(void)createView{
    ...
    [self.icon1 LabelWithIconStr:@"\U0000e87d" inIcon:iconfont andSize:CGSizeMake(25, 25) andColor:RGB(246, 15, 150) andiconSize:25];
     [self.icon2 LabelWithIconStr:@"\U0000e611" inIcon:iconfont andSize:CGSizeMake(25, 25) andColor:RGB(84, 179, 253) andiconSize:25];
    [self.icon3 LabelWithIconStr:@"\U0000e639" inIcon:iconfont andSize:CGSizeMake(20, 20) andColor:RGB(104, 255, 215) andiconSize:25];
...
//            break;
//    }
    [self.revealSideViewController pushViewController:vc onDirection:PPRevealSideDirectionNone animated:YES];
    NSDictionary *dic = @{@"id":[NSString stringWithFormat:@"%ld",(long)sender.tag]};
