...
@interface ContiRentViewController ()
@property(nonatomic,strong)NSDictionary *orderDic;
@property(nonatomic,strong)NSMutableArray *icon_LabArr;
@property(nonatomic,strong)NSString *date_Num;
@end

@implementation ContiRentViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    [self initNaviView:nil hasLeft:1 leftColor:nil title:@"续租" titleColor:nil right:@"" rightColor:nil rightAction:nil];
...
-(void)loadData
{
    ...
    [self postWithURLString:@"/user/renew_view" parameters:param success:^(id response) {
        SLog(@"详情数据%@",response);
        if(response)
        {
...
-(void)reloadView{
    NSDictionary *goodDic = self.orderDic[@"goodsinfo"];
...
- (IBAction)confirm:(UIButton *)sender {
    ...
    NSDictionary *param = @{@"user_id":[XTool GetDefaultInfo:USER_INFO][USER_ID],@"order_id":self.order_id,@"goods_id":self.good_Id,@"month":self.date_Num};
    [self postWithURLString:@"/user/renew" parameters:param success:^(id response) {
        if(response){
            [WToast showWithText:@"续租成功"];
            PayZViewController *vc = [[PayZViewController alloc]init];
            vc.orderStr = response[@"result"][@"order_sn"];
            vc.cashStr =  response[@"result"][@"order_amount"];
            vc.iscontinue = 1;
            [self pushView:vc];
