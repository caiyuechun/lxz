...
#import "RetOrderCell.h"

@implementation RetOrderCell
...
-(void)bindDatas:(NSDictionary *)diction
{
    ...
    self.orderStatus.text = diction[@"status_name"];
//    NSString *leftStr = @"";
//    NSString *rightStr = @"";
...
//    [self.handelright setTitle:rightStr forState:UIControlStateNormal];
    NSDictionary *goodDic = diction;
  //  NSString *url = [NSString stringWithFormat:@"%@%@",BASE_SERVICE,goodDic[@"original_img"]];
     NSString *url = [NSString stringWithFormat:@"%@",goodDic[@"img"]];
    [self.image sd_setImageWithURL:[NSURL URLWithString:url]];
    self.name.text = goodDic[@"reason"];
    self.status.text = diction[@"status_name"];
...
//    self.price.text = [NSString stringWithFormat:@"合计：¥%.2f",totals];
  //  self.price.text = [NSString stringWithFormat:@"合计：¥%@",diction[@"order_amount"]];
    NSString*price = [NSString stringWithFormat:@"合计：¥%@",diction[@"total_amount"]];
