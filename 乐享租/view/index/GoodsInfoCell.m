...
- (void)awakeFromNib {
    ...
    self.date_Lab.textColor = PRICE_COLOR;
    self.price_Lab.text = @"¥15/月";
    self.date_Lab.text = @"最低1个月";
    self.corner_View.backgroundColor = viewcontrollerColor;
    self.corner_View.layer.masksToBounds = 1;
    self.corner_View.layer.cornerRadius = 25;
    // Initialization code
}
- (IBAction)checkAllComent:(id)sender {
    self.checkAllcomm();
}
-(void)bindCell:(NSDictionary *)dic{
    self.name.text = dic[@"goods_name"];
    self.price_Lab.text = [NSString stringWithFormat:@"%@/月",dic[@"shop_price"]];
    self.coment_Count.text = [NSString stringWithFormat:@"%@",dic[@"comment_count"]];
    //self.date_Lab.text = [];
}
...
//    }
    ...
    self.remark.frame = CGRectMake(remarkOldRect.origin.x, heights+15, screen_width-20, remarkHei);
    
    CGRect priceOldRect = self.price.frame;
    CGRect oldPriceRect = self.oldPrice.frame;
    self.price.frame = CGRectMake(priceOldRect.origin.x, remarkHei+heights, priceOldRect.size.width, priceOldRect.size.height);
   
    
    self.oldPrice.frame = CGRectMake(oldPriceRect.origin.x, remarkHei+heights, oldPriceRect.size.width, oldPriceRect.size.height);
  //  CGRect saleOldRect = self.sales.frame;

     CGRect saleOldRect = self.sales.frame;
    self.sales.frame = CGRectMake(saleOldRect.origin.x, remarkHei+heights+15, saleOldRect.size.width, saleOldRect.size.height);
}
